...
def normalize_by_polygon(
        raster_path, vector_path, percentile, clamp_range, target_path):
    ...
    workspace_dir = tempfile.mkdtemp(dir=base_dir)
    ...
    TASK_GRAPH.join()
    percentile_pickle_path = '%s_%d.pickle' % (
        os.path.splitext(os.path.basename(raster_path))[0], percentile)
    calculate_percentile_task = TASK_GRAPH.add_task(
        func=calculate_percentile,
        args=(raster_path, [percentile], base_dir, percentile_pickle_path),
        target_path_list=[percentile_pickle_path],
        task_name='calculating %s' % percentile_pickle_path)
    calculate_percentile_task.join()
    with open(percentile_pickle_path, 'wb') as pickle_file:
        percentile_values_list = pickle.load(pickle_file)
    LOGGER.debug(percentile_values_list)
...
if __name__ == '__main__':
    ...
    normalize_by_polygon(
        RASTER_PATH, VECTOR_PATH, 97, [0, 1], TARGET_PATH)
